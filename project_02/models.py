...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, 
        ...
        ##########################################################
        # YOUR CODE HERE
        # We start by labeling class_counts to determine the most likely calss
        # then we do robustness certification
        class_counts = self._sample_noise_predictions(inputs, n0, batch_size)
        top_class = torch.argmax(class_counts)
        robu_class_counts = self._sample_noise_predictions(inputs, num_samples, batch_size)
        numA = robu_class_counts[top_class]
        p_A_lower_bound = lower_confidence_bound(num_class_A=numA, num_samples=num_samples, alpha=alpha)
        ##########################################################
        ...
        if p_A_lower_bound < 0.5:
            ...
            ##########################################################
            # YOUR CODE HERE
            # creating a normal dist. here, use it to calculate the radius
            N_D = torch.distributions.normal.Normal(torch.tensor([0.0]), torch.tensor([1.0]))
            radius = N_D.icdf(torch.tensor(p_A_lower_bound)) * self.sigma
            ##########################################################
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, 
        ...
        ##########################################################
        # YOUR CODE HERE
        top_two_classes, index = torch.sort(class_counts, descending=True)
        if binom_test(top_two_classes[0], torch.sum(top_two_classes), p=0.5) < alpha:
            return top_two_classes[0]
        else:
            return -1
        ##########################################################
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, 
        ...
        with torch.no_grad():
            classes = torch.arange(self.num_classes).to(self.device()) # K*1
            class_counts = torch.zeros([self.num_classes], dtype=torch.long, 
                                       device=self.device())    #K*1
            for it in range(ceil(num_samples / batch_size)):
                this_batch_size = min(num_remaining, batch_size)
                ##########################################################
                # YOUR CODE HERE
                # first we construct a noise for input(B*C*N*N)
                # get prediction through froward pass(B*K)
                # label the index of maximum prob. pf each SAMPLE
                # update class_counts with the num of each class
                noise = inputs.repeat(this_batch_size, 1, 1, 1)
                predictions = self.forward(noise)
