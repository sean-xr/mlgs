...
def train_model(model: nn.Module, dataset: Dataset, batch_size: int, 
    ...
    num_train_batches = int(torch.ceil(num_train_batches).item()) #向上取整，防止有余数
    for epoch in range(epochs):
        train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
        for x,y in tqdm(train_loader, total=num_train_batches):
            ##########################################################
            # YOUR CODE HERE
            loss_one_sample, logits = loss_function(x, y, model, **loss_args) #actually this should be loss_one_batch
            optimizer.zero_grad()
            #before optimizing make the gradients to be 0
            torch.autograd.backward(loss_one_sample)
            #optimize the weights
            optimizer.step()
            # find the index of the largest prob. of each sample
            predictions = torch.argmax(logits, dim=1)
            # calculating accury w.r.t. each batch
            accuracy_one_batch = torch.sum(predictions == y) / batch_size
            losses.append(loss_one_sample)
            accuracies.append(accuracy_one_batch)
            ##########################################################
...
def predict_model(model: nn.Module, dataset: Dataset, batch_size: int, 
    ...
    for x, y in tqdm(test_loader, total=num_batches):
        ...
        x.requires_grad_()
        logits = model(x)
        # first: discuss whether there is a attack
        if attack_function is not None:
            x_tertia = attack_function(logits, x, y, **attack_args)
            logits_tertia = model(x_tertia)  # B*K
            prediction_per_batch = torch.argmax(logits_tertia,axis=1) #shape(B)
            predictions.append(prediction_per_batch)
        else:
            prediction_per_batch = torch.argmax(logits, axis=1)
            predictions.append(prediction_per_batch)
        targets.append(y)
        ################################################
        # ##########
    ...
    accuracy = (predictions == targets).float().mean().item()  #shape is in a line, convenient for accuracy
    return accuracy
...
def evaluate_robustness_smoothing(base_classifier: nn.Module, 
    ...
    for x, y in tqdm(test_loader, total=len(dataset)):
        ##########################################################
        # YOUR CODE HERE
        top_class, radius = model.certify(inputs=x, n0=num_samples_1, num_samples=num_samples_2, alpha=alpha,
